...
Node::~Node()
{
    ...
    _Private* priv = reinterpret_cast<_Private*>(_xml->_private);
    if ( priv->__sig != _REFERMEE_XML_SIGNATURE || priv->__ptr.get() != this )
        return;
...
void Node::Unwrap(_xmlNode *aNode)
{
    ...
    if (aNode->type == XML_NAMESPACE_DECL)
    {
        ...
        if (__ns->_private != nullptr)
        {
            ...
            if (ptr->__sig == _REFERMEE_XML_SIGNATURE)
            {
                ptr->__ptr->release();
                delete ptr;
            }
...
void Node::rebind(_xmlNode *newNode)
{
    ...
    if (_xml->_private != nullptr && *((unsigned int*)(_xml->_private)) == _REFERMEE_XML_SIGNATURE)
    {
