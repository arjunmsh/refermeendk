...
static CONSTEXPR const unsigned int _REFERMEE_XML_SIGNATURE = 0x52586D6C;    /* 'RXml' */
...
{
    LibXML2Private()
        : __sig(_REFERMEE_XML_SIGNATURE), __ptr(nullptr)
        {}
    LibXML2Private(_Tp* __p)
        : __sig(_REFERMEE_XML_SIGNATURE), __ptr(__p)
        {}
    ...
    // data member-- used to determine if this is a Refermee-made pointer
    unsigned int __sig;
...
#define IS_REFERMEE_WRAPPED_XML(xml) (((xml) != nullptr) && ((xml)->_private != nullptr) && (*((unsigned int*)xml->_private) == _REFERMEE_XML_SIGNATURE))
...
static inline std::shared_ptr<_Tp> Wrapped(_Nm * __n)
{
    ...
    try
    {
        ...
        if (__n->_private != nullptr && __n->_private > (void*)0x1000)
        {
            ...
            if (__p->__sig == _REFERMEE_XML_SIGNATURE)
            {
                return __p->__ptr;
            }
            else
...
static inline void Rewrap(_Nm* __n, std::shared_ptr<_Tp> __t)
{
    ...
    try
    {
        if (IS_REFERMEE_WRAPPED_XML(__n))
        {
